<template>
  <section class="w-full px-4 py-6 bg-Ghibli-skin/20">
    <!-- 居中容器 -->
    <div class="max-w-5xl mx-auto">
      <!-- 標題 -->
      <h2 class="text-xl font-semibold text-Ghibli-brown mb-4 flex items-center">
        <svg
          class="w-6 h-6 mr-2 text-Ghibli-blue"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
      ...
      <!-- 學期選擇 -->
      <div class="card mb-6">
        <label class="form-label">選擇學期</label>
        <select
          v-model="selectedSemester"
          class="form-input"
        >
          <option value="2025-1">2025學年第一學期</option>
          <option value="2025-2">2025學年第二學期</option>
        </select>
      </div>
      ...
      <div
        ...
        class="card bg-white p-4 text-Ghibli-light-red"
      >
      ...
      >
        ...
        <div class="lg:col-span-6">
          <div class="space-y-6">
            ...
            <button
              @click="handleSubmit"
              class="w-full px-4 py-2 bg-Ghibli-blue text-white rounded-lg font-semibold hover:bg-Ghibli-blue/80 transition-colors flex items-center justify-center"
            >
              <svg
                class="w-5 h-5 mr-2"
                fill="none"
      ...
      <div
        v-if="showErrorModal"
        class="fixed inset-0 bg-Ghibli-brown/50 flex items-center justify-center z-50"
      >
        <div class="card bg-white p-6 max-w-sm w-11/12">
          <h3 class="text-lg font-semibold text-Ghibli-brown mb-4">{{ errorMessage }}</h3>
          <button
            @click="showErrorModal = false"
            class="px-4 py-2 bg-Ghibli-blue text-white rounded-lg hover:bg-Ghibli-blue/80"
          >
            確定
          </button>
        </div>
